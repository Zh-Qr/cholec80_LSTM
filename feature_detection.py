            ...
            with open(ann_file, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    try:
                        ...
                        continue
                    annotations.append(torch.tensor(labels, dtype=torch.long))
...
def train(model, train_loader, criterion, optimizer, epoch, device):
    ...
    for batch_idx, (inputs, targets) in loop:
        # inputs: [batch_size, window_size, feature_dim]
        # targets: [batch_size, num_classes]（one-hot 平均表示）
        inputs, targets = inputs.to(device), targets.to(device)
        
        _, outputs = model(inputs)  # 输出 logits: [batch_size, num_classes]
        ...
        loop.set_postfix(loss=running_loss/(batch_idx+1), accuracy=100. * correct / total)
    
    avg_loss = running_loss / len(train_loader)
    accuracy = 100. * correct / total
    return avg_loss, accuracy

def start_train(num_epochs, save_name, model, train_loader, criterion, optimizer, scheduler, device):
